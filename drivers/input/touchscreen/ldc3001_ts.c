...
#define MAX_PAYLOAD_SIZE	((MAX_TOUCH_POINTS * TP_READ_SIZE) \
		+ DEFAULT_READ_LEN)
					...
					const struct i2c_device_id *idev_id)
{
	...
	/* TODO: Get the MAX_TOUCH_POINTS value from dt node */
	if (data) {
		data->message = devm_kzalloc(&client->dev,
				(sizeof(u8) * MAX_PAYLOAD_SIZE), GFP_KERNEL);
		input_dev = devm_input_allocate_device(&client->dev);
	}
	if (!data || !input_dev || !data->message) {
